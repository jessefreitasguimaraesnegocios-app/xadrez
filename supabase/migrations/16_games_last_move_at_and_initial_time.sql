-- Tempo oficial no servidor: last_move_at (sempre gerado pelo servidor)
ALTER TABLE public.games
  ADD COLUMN IF NOT EXISTS last_move_at TIMESTAMPTZ;

-- Inicializar last_move_at nos jogos existentes a partir de started_at
UPDATE public.games
SET last_move_at = started_at
WHERE last_move_at IS NULL AND started_at IS NOT NULL;

-- create_match_atomic: definir white_remaining_time, black_remaining_time e last_move_at
-- time_control "X+Y" => X minutos iniciais (ignoramos incremento por simplicidade)
...
BEGIN
  ...
  -- Tempo inicial em segundos: "X+Y" => X * 60
